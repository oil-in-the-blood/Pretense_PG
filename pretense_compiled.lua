...
pretenseLog = mist.Logger:new('PretenseLog',3)
...
Config.supplyBuildSpeed = Config.supplyBuildSpeed or 185 -- supply helicopters and convoys build speed
...
do
	...
	function CustomZone:getRandomBuildingZone()
		local spawnZones = {}
		for i=1,100,1 do
			local zname = self.name..'-b-'..i
			if trigger.misc.getZone(zname) then
				if not self.bzonesUsed[zname] then
					table.insert(spawnZones, zname)
				end
			else
				break
		...
		if #spawnZones == 0 then
			env.info('CustomZone:getRandomBuildingZone :: no building zones found for: '..self.name)
			return self:getRandomSpawnZone()
		else
			local choice = math.random(1,#spawnZones)
			local cName = spawnZones[choice]
			if cName then
				env.info('CustomZone:getRandomBuildingZone :: building zone '..cName..' selected in zone '..self.name)
			else
				env.info('CustomZone:getRandomBuildingZone :: PROBLEM with spawnZones[choice] for zone: '..self.name)
			end
			table.insert(self.bzonesUsed,{[cName]=true})
			return spawnZones[choice]
		end
	end
	
	function CustomZone:getRandomModZone(modifier)
		local spawnZones = {}
		for i=1,100,1 do
			local zname = self.name..'-'..modifier..'-'..i
			if trigger.misc.getZone(zname) then
					table.insert(spawnZones, zname)
			else
				break
	...
	function CustomZone:spawnGroup(product, acceptedSurface)
		...
		if not spawnzone then
			if product.spawnMod then
				spawnzone = self:getRandomModZone(product.spawnMod)
			elseif product.type == 'upgrade' then
				spawnzone = self:getRandomBuildingZone()
			else
		...
		env.info('CustomZone:spawnGroup :: Spawn Zone --'..spname..'-- picked for '..product.name)

		if product.spawnMod then
			if product.spawnMod == 'naval' then
				acceptedSurface = { [land.SurfaceType.WATER] = true }
			end
		end
...
do
	...
	function GroupMonitor:registerGroup(product, target, home, savedData)
		...
		env.info('GroupMonitor - registerGroup ['..product.name..'] with .name = '..self.groups[product.name].name)
	...
	function GroupMonitor:start()
		timer.scheduleFunction(function(param, time)
			local self = param.context
			env.info("GroupMonitor:start Running, self.groups size = "..#self.groups)
			for i,v in pairs(self.groups) do
				local isDead = false
				if v.product.missionType == 'supply_convoy' or v.product.missionType == 'assault' or v.product.missionType == 'supply_naval' then
	...
	function GroupMonitor:processSurface(group) -- states: [started, enroute, atdestination, siege]
		...
		elseif group.state =='enroute' then
			if gr then
				...
				if z and (z.name==group.target.name or z.name==group.home.name) then
					...
					if group.product.missionType == 'supply_convoy' or group.product.missionType == 'supply_naval' then
					...
					elseif group.product.missionType == 'supply_naval' then
						if GroupMonitor.isStuck(group) then
							group.unstuck_attempts = group.unstuck_attempts or 0
							group.unstuck_attempts = group.unstuck_attempts + 1
							if (group.unstuck_attempts > 10) then
								env.info('GroupMonitor: Ship '..group.name..' has beached, sinking it')
								gr:destroy()
								return true
							end
						end
		...
		elseif group.state == 'atdestination' then
			env.info(group.name..'In state atdestination')
...
do
	...
	function TaskExtensions.moveOnWaterToPoint(group, routeList)
		if not group or not routeList then return end
		if not group:isExist() or group:getSize()==0 then return end
		...
		env.info("TaskExtensions:moveOnWaterToPoint "..group:getName().." route list is "..#routeList.." points")
		--Build the Route
		for k,v in pairs(routeList) do
		
			local zoneWP = trigger.misc.getZone(v)
			
			--Verify that the zone exists
			if zoneWP then
				local zonePoint = zoneWP.point
				table.insert(mis.params.route.points, {
					type= AI.Task.WaypointType.TURNING_POINT,
					x = zonePoint.x,
					y = zonePoint.z,
				...
				--env.info(group.name..' Waypoint Zone Name '..v..' does not exist')
			end
		end
		
		group:getController():setTask(mis)
		env.info("TaskExtensions:moveOnWaterToPoint assigned waypoints to "..group:getName())
		env.info(group:getController())
	...
	function TaskExtensions.landAtPoint(group, point, alt, skiptakeoff) -- point = {x,y}
		...
		local slope = (point.y - startPos.z)/(point.x-startPos.x )
		local plusC = point.y - (slope*point.x)
		
		local outsideX = point.x
		local outsideY = point.y
		
		local offsetAmt = 3000
		local offsetAmtY = offsetAmt
		local offsetAmtX = offsetAmt
		
		if point.y < startPos.z then
			offsetAmtY = -1*offsetAmt
		end
		
		if point.x < startPos.x then
			offsetAmtX = -1*offsetAmt
		end
		
		
		if slope > 1 then
			outsideY = point.y - offsetAmtY
			outsideX = (outsideY - plusC)/slope 
		else
			outsideX = point.x - offsetAmtX
			outsideY = slope * outsideX + plusC
		end
...
do
	...
	function ZoneCommand:start()
		timer.scheduleFunction(function(param, time)
			...
			--env.info('Zone Commander Loop Function for '..self.name)
	...
	function ZoneCommand:verifyBuildValid()
		if self.currentBuild then
			env.info('ZoneCommand:verifyBuildValid :: Checking if '..self.currentBuild.product.name..' is still valid')
			if self.side == 0 then
				self.currentBuild = nil 
			...
			if self.mode == 'export' or self.mode == 'supply' then 
				if not (self.currentBuild.product.type == ZoneCommand.productTypes.upgrade or 
					...
					self.currentBuild.product.missionType == ZoneCommand.missionTypes.supply_naval or
		...
		if self.currentMissionBuild then
			env.info('ZoneCommand:verifyBuildValid :: Checking if '..self.currentMissionBuild.product.name..' is still valid')
			if self.side == 0 then
				self.currentMissionBuild = nil 
				env.info('ZoneCommand:verifyBuildValid - stopping mission build, zone is neutral')
			end

			if (self.mode == 'export' and not self.keepActive) or (self.mode == 'supply' and self.currentMissionBuild.product.missionType ~= ZoneCommand.missionTypes.naval_patrol) then
		...
		if not self.currentMissionBuild and not self.currentBuild then
			env.info('ZoneCommand:verifyBuildValid has no current build/mission to check')
		end
	end
	...
	function ZoneCommand:chooseBuild()
		...
		--local treshhold = 0
	--	env.info('ZoneCommand:chooseBuild() for '..self.name)
		if self.side ~= 0 and self.currentBuild == nil then
		--	env.info(self.name..':chooseBuild(): My side is non-zero and I have no currentbuild')
			local canAfford = {}
			for _,v in ipairs(self.upgrades[self.side]) do
			--	env.info(self.name..':chooseBuild(): Checking for upgrade: '..v.name)
				...
				elseif u ~= nil then
					for _,v2 in ipairs(v.products) do
						--env.info(self.name..':chooseBuild(): Checking Mission'..v2.name.. ' for '..v.name)
						if v2.type == 'mission'  then 
							if self.resource > treshhold and
								...
								v2.missionType == ZoneCommand.missionTypes.supply_naval or
								v2.missionType == ZoneCommand.missionTypes.supply_transfer) then
								--env.info(self.name..':chooseBuild(): Mission'..v2.name.. ' for '..v.name..' is: '..self:isMissionValid(v2))
								if self:isMissionValid(v2) then --and math.random() < ZoneCommand.missionValidChance then
									env.info(self.name..':chooseBuild(): Mission'..v2.name..'is valid adding to canAfford')
									...
									env.info(self.name..':chooseBuild(): Mission'..v2.name..'is not valid doesnt go in canAfford')
								end
							end
	...
	--		env.info(self.name..':chooseBuild(): I can afford '..#canAfford..' things')
			if #canAfford > 0 then
				
				local choice = math.random(1, #canAfford)
			--	env.info(self.name..':chooseBuild(): My choice is '..choice)
		...
		--	env.info(self.name..':chooseBuild(): My side is non-zero and I have no currentMissionBuild')
			local canMission = {}
			for _,v in ipairs(self.upgrades[self.side]) do
	--			env.info(self.name..':chooseBuild(): Checking Mission(s) for'..v.name)
				local u = Group.getByName(v.name)
				if not u then u = StaticObject.getByName(v.name) end
				if u ~= nil then
					for _,v2 in ipairs(v.products) do
			--			env.info(self.name..':chooseBuild(): Checking Mission'..v2.name.. ' for '..v.name)
						if v2.type == 'mission' then 
							if v2.missionType ~= ZoneCommand.missionTypes.supply_air and 
								...
								v2.missionType ~= ZoneCommand.missionTypes.supply_naval and
								v2.missionType ~= ZoneCommand.missionTypes.supply_transfer then
		...
		--	env.info(self.name..':chooseBuild(): I can do '..#canMission..' Mission(s)')
	...
	function ZoneCommand:progressBuild()
		if not self.currentBuild then
			env.info('ZoneCommand:progressBuild :: no build currently for '..self.name)
		end
		
		if not self.currentMissionBuild then
			env.info('ZoneCommand:progressBuild :: no mission build currently for '..self.name)
		end
		...
		if self.currentMissionBuild then
			env.info('ZoneCommand:progressBuild :: current mission build is '..self.currentMissionBuild.product.name..' for '..self.name)
			if self.currentMissionBuild.product.type == 'mission' and not self:isMissionValid(self.currentMissionBuild.product) then
				...
				env.info('ZoneCommand:progressBuild :: build progress: '..self.currentMissionBuild.progress..' / '..cost)
	...
	function ZoneCommand:queueBuild(product, side, isRepair, progress)
		if product.type ~= ZoneCommand.productTypes.mission or 
			...
			product.missionType == ZoneCommand.missionTypes.supply_naval or
	...
	function ZoneCommand:isMissionValid(product)
		env.info(product.name..': Checking for valid mission')
		
		if Group.getByName(product.name) then return false end
		env.info(product.name..': Group OK')
		
		if self.reservedMissions[product.name] then
			return false
		end
		env.info(product.name..': Not Reserved')
		
		if product.missionType == ZoneCommand.missionTypes.supply_convoy then
--			env.info(product.name..': Is Supply Convoy')
			
			if self.distToFront == nil then return false end
--			env.info(product.name..': Distance to Front OK')

			for _,tgt in pairs(self.neighbours) do
--				env.info(product.name..': Checking Target: '..tgt.name)
				if self:isSupplyMissionValid(product, tgt) then 
--					env.info(product.name..': Target: '..tgt.name..' Is Good')
					return true 
				end
--				env.info(product.name..': Target: '..tgt.name..' Bad')
			end
		elseif product.missionType == ZoneCommand.missionTypes.supply_naval then
			env.info(product.name..': Is Supply Naval')
			...
			for _,tgt in pairs(self.neighbours) do
				env.info(product.name..': Checking Target: '..tgt.name)
				if self:isNavalMissionValid(product, tgt) then 
					env.info(product.name..': Target: '..tgt.name..' Is Good')
					return true 
				end
				env.info(product.name..': Target: '..tgt.name..' Bad')
...
--			env.info(product.name..': Is Supply Transfer')
...
--				env.info(product.name..': Target: '..tgt.name..' No Good')
			end
		...
		elseif product.missionType == ZoneCommand.missionTypes.naval_patrol then
			if self.mode ~= ZoneCommand.modes.export then 
				env.info('Naval Patrol Valid')
				return true
			else
				env.info('Naval Patrol Not Valid')
				return false 
			end
		end
	...
	function ZoneCommand:activateMission(product)
		...
		elseif product.missionType == ZoneCommand.missionTypes.supply_naval then
			self:activateNavalSupplyMission(product)
		elseif product.missionType == ZoneCommand.missionTypes.naval_patrol then
			self:activateNavalPatrolMission(product)
		end
	...
	function ZoneCommand:reActivateMission(savedData)
		...
		elseif product.missionType == ZoneCommand.missionTypes.supply_naval then
			self:reActivateNavalSupplyMission(product, savedData)
		elseif product.missionType == ZoneCommand.missionTypes.cas then
	...
	function ZoneCommand:isBaiMissionValid(product, tgtgroup)
		...
		if tgtgroup.product.missionType == ZoneCommand.missionTypes.supply_naval then return true end
		if tgtgroup.product.missionType == ZoneCommand.missionTypes.naval_patrol then return false end
	end
	...
	function ZoneCommand:isSupplyMissionValid(product, target)
		...
		if target.side == self.side then 
			if (not product.naval) and self.distToFront > 1 and target.distToFront > self.distToFront then return false end -- skip regular missions if not close to front
	...
	function ZoneCommand:activateNavalSupplyMission(product)
		local tgtzones = {}
		navalSupplyLogger = mist.Logger:new("Naval Supply Logger","info")
		navalSupplyLogger:msg("activateNavalSupplyMission")
		if not product then
			navalSupplyLogger:msg("Product is Empty")
		end
		
		navalSupplyLogger:msg("Does the product have a name? $1",product.name)
		navalSupplyLogger:msg("product for naval supply $1",product)
		for k,v in pairs(product.navalWP) do
			navalSupplyLogger:info("Name for Naval Waypoints $1",k)
			--local allZn = self:getAllZones()
			--navalSupplyLogger:info("Target Zone?: $1",ZoneCommand.allZones[k])
			local navalZone = ZoneCommand.allZones[k]
			if navalZone then
				navalSupplyLogger:info("Success found zone info for: $1",navalZone.name)
				if (navalZone.side == 0 or navalZone.side==product.side) then
					table.insert(tgtzones, navalZone)
				end
			else
				navalSupplyLogger:info("Fail zone info not found for: $1 ",k)
			end
		end
		
		if #tgtzones == 0 then 
			env.info('ZoneCommand:activateNavalSupplyMission - '..self.name..' no valid tgtzones')
			return 
		end
		...
		--if BattlefieldManager and BattlefieldManager.priorityZones[self.side] then
		--	local prioZone = BattlefieldManager.priorityZones[self.side]
		--	if prioZone.side == 0 and self.neighbours[prioZone.name] and self:isNavalMissionValid(product, prioZone) then
		--		tgtzones = { prioZone }
		--	end
		--end
		
		for i,v in ipairs(tgtzones) do
			--verify that there is a WP list for this zone
			if self:isNavalMissionValid(product, v) then
				local og = Utils.getOriginalGroup(product.name)
				if og then
					teleportToPos(product.name, {x=og.x, z=og.y})
					env.info("ZoneCommand - activateSupplyConvoyMission teleporting to OG pos")
				...
				timer.scheduleFunction(function(param)
					local gr = Group.getByName(param.name)
					TaskExtensions.moveOnWaterToPoint(gr,param.navalWP)
				end, {name=product.name, navalWP=product.navalWP[v.name]}, timer.getTime()+5)
				
				env.info("ZoneCommand - "..product.name.." naval targeting "..v.name)
			end
			break
		end
	end
	
	function ZoneCommand:isNavalMissionValid(product, tgt) 
		--Provided a target zone (ZoneCommand object)
		if tgt then
	--		env.info("Naval Target Check for: "..tgt.name)
			--There's waypoints for the target
			if product.navalWP[tgt.name] then 
				local amtDispatched = self:numOfNavalSupplyDispatched(tgt.name)*product.cost
				local theoryAmt = amtDispatched + product.cost + tgt.resource
		
				if (theoryAmt < tgt.maxResource) then
					return true
				else return false end				
			else return false end
		--No Zone Provided, check to see if a patrol mission and a patrol WP exists
		else
			if prodcut.navalWP["patrol"] then return true
			else return false end
		end
	end
	
	function ZoneCommand:numOfNavalSupplyDispatched(tgtName)
		local count = 0
	
		for _,v in ipairs(self.upgrades[self.side]) do
			for _,v2 in ipairs(v.products) do
				--env.info("ZoneCommand:numOfNavalSupplyDispatched :: Checking Mission "..v2.name)
				--env.info("ZoneCommand:numOfNavalSupplyDispatched :: GroupMonitor.groups size = "..#DependencyManager.get("GroupMonitor").groups)
				if v2.missionType == ZoneCommand.missionTypes.supply_naval then
					--env.info("ZoneCommand:numOfNavalSupplyDispatched :: "..v2.name.." is a naval supply mission")
					for _,v3 in pairs(DependencyManager.get("GroupMonitor").groups) do
						--env.info('Looking at product: '..v3.product.name..' in Group Monitor groups')
						--Is it in the Group Monitor
						if v3.product.name == v2.name then
							--env.info("ZoneCommand:numOfNavalSupplyDispatched :: Found "..v2.name.." In the Group Monitor")
							--env.info("ZoneCommand:numOfNavalSupplyDispatched :: Target is "..v3.target.name)
							--Check if the target is the requested target
							if v3.target.name == tgtName then
								count = count + 1
							end
						else
							--env.info("ZoneCommand:numOfNavalSupplyDispatched :: Didn't Find "..v2.name.." In the Group Monitor")
						end
					end
				end
			end
	...
	function ZoneCommand:reActivateNavalSupplyMission(product, savedData)
		local zone = ZoneCommand.getZoneByName(savedData.lastMission.zoneName)
		local wpListTarget = product.navalWP[savedData.lastMission.zoneName]
		env.info('ZoneCommand:reActivateNavalSupplyMission :: '..product.name..' Assigned for Reactivate')
		--make sure the target exists and there are waypoints to it
		if zone and wpListTarget then
			--Move Unit to last point
			env.info('ZoneCommand:reActivateNavalSupplyMission :: Teleporting '..product.name..' To Last Position')
			mist.teleportToPoint(getDefaultPos(savedData, false))
			DependencyManager.get("GroupMonitor"):registerGroup(product, zone, self, savedData)
			product.lastMission = {zoneName = zone.name}
		
			--Need to modify waypoint route.
			myLogger = mist.Logger:new('MyLog',2)
			myLogger:info('wpListTarget: $1',wpListTarget)
			local modWpList = mist.utils.deepCopy(wpListTarget)
			env.info('ZoneCommand:reActivateNavalSupplyMission :: '..product.name..' Has '..#wpListTarget.." Waypoints in route")
			--Need at least 2 waypoints
			if #wpListTarget > 1 then
				local p0 = nil
				local p1 = nil
				for i,v in ipairs(wpListTarget) do
					env.info('ZoneCommand:reActivateNavalSupplyMission :: indexing waypoint'..i..' Waypoint: '..v)
					if p0 then
						p1 = mist.utils.makeVec3GL(trigger.misc.getZone(v).point)
						
						local distToP0 = mist.utils.get2DDist(p0,savedData.position)
						local distToP1 = mist.utils.get2DDist(p1,savedData.position)
						local distP0ToP1 = mist.utils.get2DDist(p0,p1)
						
						myLogger:info('ship position: $1',savedData.position)
						myLogger:info('p0 position: $1',p0)
						myLogger:info('p1 position: $1',p1)
						myLogger:info('distToP0 position: $1',distToP0)
						myLogger:info('distToP1 position: $1',distToP1)
						myLogger:info('distP0ToP1 position: $1',distP0ToP1)
						
						
						--env.info('ZoneCommand:reActivateNavalSupplyMission :: ship position: '..savedData.position)
						--env.info('ZoneCommand:reActivateNavalSupplyMission :: p0 position: '..p0)
						--env.info('ZoneCommand:reActivateNavalSupplyMission :: p1 position: '..p1)
						--env.info('ZoneCommand:reActivateNavalSupplyMission :: distToP0: '..distToP0)
						--env.info('ZoneCommand:reActivateNavalSupplyMission :: distToP1: '..distToP1)
						--env.info('ZoneCommand:reActivateNavalSupplyMission :: distP0ToP1: '..distP0ToP1)
						--     A-----p(n)----B-----p(n+1)----C
						--
						--     If at Point A, don't trim the table anymore, and send it starting at p(n)
						--     If at Point B, trim out p(n) and sent it
						--	   If at Point C, tirm out p(n) and iterate n (p(n+1) become p(n) and p(n+2) is not p(n+1) on next loop)
						
						
						-- At Position A
						if (distToP0 < distToP1) and (distToP1 > distP0ToP1) then
							break
						
						--At Position C
						elseif (distToP0 > distP0ToP1) and (distToP0 > distToP1) then 
							table.remove(modWpList,1)
						
						--Not at Position A or C, so assume at position B
						else
							table.remove(modWpList,1)
							break						
						end
						...
						p0 = mist.utils.makeVec3GL(trigger.misc.getZone(v).point)
					end
				end
			end
			env.info('ZoneCommand:reActivateNavalSupplyMission :: '..product.name..' Has '..#modWpList.." Waypoints in modified route")
			timer.scheduleFunction(function(param)
					local gr = Group.getByName(param.name)
					TaskExtensions.moveOnWaterToPoint(gr,param.navalWP)
			end, {name=product.name, navalWP=modWpList}, timer.getTime()+5)
	...
	function ZoneCommand:activateNavalPatrolMission(product)
		if product.navalWP then
			local og = Utils.getOriginalGroup(product.name)
			if og then
				teleportToPos(product.name, {x=og.x, z=og.y})
				env.info("ZoneCommand - activateNavalPatrolMission teleporting to OG pos")
			else
				mist.respawnGroup(product.name, true)
				env.info("ZoneCommand - activateNavalPatrolMission fallback to respawnGroup")
			end
			DependencyManager.get("GroupMonitor"):registerGroup(product, v, self)
			timer.scheduleFunction(function(param)
				local gr = Group.getByName(param.name)
				TaskExtensions.moveOnWaterToPoint(gr,param.navalWP)
			end, {name=product.name, navalWP=product.navalWP['patrol']}, timer.getTime()+5)
				
			env.info("ZoneCommand - "..product.name.." naval patrol moving out")
	...
	function ZoneCommand:isPatrolMissionValid(product, target)
		--if target.side ~= product.side then return false end
		if target.name == self.name then return false end
		if not target.distToFront or target.distToFront > 5 then return false end
...
do
    ...
    function PlayerTracker:init()
        ...
        function ev:onEvent(event)
            ...
            if event.id == world.event.S_EVENT_KILL then
				...
				env.info("KILLED EVENT: "..target:getName().." was killed");
			...
			if event.id == world.event.S_EVENT_BIRTH and event.initiator and event.initiator.getPlayerName then
				...
				if player then
                    ...
                    if not context.groupShopMenus[groupid] then
						...
						missionCommands.addCommandForGroup(groupid, 'Spawn Armor Column [5000 Resources]', menu, Utils.log(context.spawnArmor), context, groupname)
	...
	function PlayerTracker:spawnArmor(groupname)
		local gr = Group.getByName(groupname)
		if gr and gr:getSize() > 0 then
			...
			self.groupTgtMenus[gr:getID()] = MenuRegistry.showTargetZoneMenu(gr:getID(), "Artillery target", function(params)
																			CommandFunctions.spawnArmorZone(params.zone)
																			end, 2, 1)

            if self.groupTgtMenus[gr:getID()] then
                trigger.action.outTextForGroup(gr:getID(), "Select spawn from radio menu",10)
            else
                trigger.action.outTextForGroup(gr:getID(), "No valid spawns available",10)
            end
...
do
    ...
    ReconManager.aircraftStats = {
		...
		['OH58D'] =         { recon_speed = 10, minDist = 15, maxDeviation = 120 },
...
do
	...
	TemplateDB.templates["storehouse"] = { type="Building02_PBR", category="Fortifications", shape="M92_Building02_PBR", dataCategory=TemplateDB.type.static } 
	
	TemplateDB.templates["watch-tower"] = { type="Sandbag_13", category="Fortifications", shape="M92_Sandbag_13", dataCategory=TemplateDB.type.static } 
	
	TemplateDB.templates["hard-air-bunker"] = { type="Shelter", category="Fortifications", shape="ukrytie", dataCategory=TemplateDB.type.static }
	
	TemplateDB.templates["new-barracks"] = { type="Building08_PBR", category="Fortifications", shape="M92_Building08_PBR", dataCategory=TemplateDB.type.static }
	
	TemplateDB.templates["new-factory"] = { type="Building03_PBR", category="Fortifications", shape="M92_Building03_PBR", dataCategory=TemplateDB.type.static } 
	
	TemplateDB.templates["new-tent"] = { type="Tent01", category="Fortifications", shape="M92_Tent01", dataCategory=TemplateDB.type.static } 
	
	TemplateDB.templates["shipment-storehouse"] = { type="Building04_PBR", category="Fortifications", shape="M92_Building04_PBR", dataCategory=TemplateDB.type.static } 
	
	TemplateDB.templates["large-garage"] = { type="Building05_PBR", category="Fortifications", shape="M92_Building05_PBR", dataCategory=TemplateDB.type.static } 
	
	TemplateDB.templates["fuel-truck"] = { type="r11_volvo", category="Fortifications", shape="M92_r11_volvo", dataCategory=TemplateDB.type.static } 
	
	TemplateDB.templates["small-garage"] = { type="Building01_PBR", category="Fortifications", shape="M92_Building01_PBR", dataCategory=TemplateDB.type.static } 
	
	TemplateDB.templates["generator"] = { type="HESCO_generator", category="Fortifications", shape="M92_HESCO_generator", dataCategory=TemplateDB.type.static } 
	
	TemplateDB.templates["dock-crane"] = { type="Tower Crane", category="Fortifications", shape="TowerCrane_01", dataCategory=TemplateDB.type.static } 
	
	TemplateDB.templates["offshore-oil"] = { type="Oil platform", category="Fortifications", shape="plavbaza", dataCategory=TemplateDB.type.static } 
	
	TemplateDB.templates["offshore-gas"] = { type="Gas platform", category="Heliports", shape="gas_platform", dataCategory=TemplateDB.type.static } 
...
do
    ...
    function Spawner.createObject(name, objType, pos, side, minDist, maxDist, surfaceTypes, zone)
		...
		elseif data.dataCategory == TemplateDB.type.groupNaval then
		    return coalition.addGroup(cnt, Group.Category.SHIP, data)
        end
    end
